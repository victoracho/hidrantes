...
?>
	...
	<div id="content">
		...
		<div class="header-doc">
			<h1><span>Ver ficha Hidrante</span></h1>	
            <div class="clr"><hr /></div>
			<p class="head-button"><a href="<?php echo $admin_path?>hidrantes/informes/?&page=<?=$page?>"><span><?=$str_lang['LANG_HIDRANTES_INFORMES_VOLVER']?></span></a></p>
			<?php
...
<div class="clr"><hr /></div>						
	...
	$lista_comentarios=$bd->get_all_by_filter("V_Comentarios","hidrante_id=".$id);
	
				?>
				
				<div class="orders">	
					
					<h2><?=$str_lang['LANG_HEADER_TAB_COMENTARIOS']?></h2>	
					<?php
					if (empty($lista_comentarios))
					{
						echo "<br/><br/><h3>".$str_lang['LANG_MSG_ERROR18']."</h3>";
					}
					...
					?>					
					<table id="table-list-tasas">						
						<tbody>			
							<tr>
								<td class="title" ><?=$str_lang['LANG_USERS_COL_USER']?>:</td><td width="120px"> <?=$val['Name']?></td>
								<td class="title"  ><?=$str_lang['LANG_HIDRANTES_FECHA']?>:</td><td width="90px">  <?=convert_date($val["fecha"])?></td>
								<td class="title"  ><?=$str_lang['LANG_COMENTARIOS_ASOCIADO']?>:</td><td width="90px"> <u><?=$val['asociado']?></u></td>
							</tr>	
							<tr>
								<td class="title" ><?=$str_lang['LANG_HEADER_TAB_COMENTARIOS']?>:</td>
								<td colspan="5" ><textarea id="comentario" name="comentario"  class="lang-edit" disabled><?=$val["comentario"]?></textarea></td>
							</tr>								
