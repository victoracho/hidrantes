...
function ponDatosComentario($vector,$i,$j)
{
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		->setCellValue('C'.$j,  utf8_encode($vector[$i]['asociado']))
...
$incComentarios=false;
if ($_POST["comentarios"]=="1")
		$incComentarios=true;
...
{
//header("Location:".$admin_path."hidrantes/excel/");
//exit;
echo '<p>'.$str_lang['LANG_MSG_ERROR24'].'</p>';
exit;
}
$j=2;
$objUTM="";
...
for($i=0;$i<$n;$i++)
{
	if ($incComentarios)
	{
		if ($vector[$i]['hidrante_id'] != $hidranteIni)  // Ponemos cabecera y datos de hidrante
		{
			$j++;
			poneCabecera($j);
			$j++;
			...
			poneCabeceraComentarios($j);
			$j++;
			$hidranteIni= $vector[$i]['hidrante_id'] ;
			
			
		}
		
		
		ponDatosComentario($vector,$i,$j);
		$j++;		
	}
	else{
		if ($vector[$i]['hidrante_id'] != $hidranteIni)
		{	
...
$objWriter->save(uploadpathadmin.'export/hidrantesPorFecha.xls');
