...
$objPHPExcel->getActiveSheet()->getStyle('A1:O1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FFCCCCCC');
...
function poneCabecera($j)
{
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('I'.$j, 'Tipo de Hidrante')
				->setCellValue('J'.$j, 'Diámetros')
				->setCellValue('K'.$j, 'Racores')
				->setCellValue('L'.$j, 'Señalizado')
				->setCellValue('M'.$j, 'Desde red exterior')
				->setCellValue('N'.$j, 'Presión caudal adecuado')
				->setCellValue('O'.$j, 'Estado general adecuado');

				
				
		$objPHPExcel->getActiveSheet()->getStyle("A$j:O$j")->getFont()->setBold(true);			
...
function ponDatosHidrante($vector,$i,$j)
{
	...
	$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('L'.$j, $seny)
		->setCellValue('M'.$j, $red)
		->setCellValue('N'.$j, $pres)
		->setCellValue('O'.$j, $est);
	...
	$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue('I'.$j, $vector[$i]['tipohidrante'])
			->setCellValue('J'.$j, $vector[$i]['diametro'])
			->setCellValue('K'.$j, $vector[$i]['racor']);
...
if ($municipio_id != "-1")
			$where .= " and municipio_id=".$municipio_id;		
...
$objWriter->save(uploadpathadmin.'export/hidrantesPorMunicipio.xls');
...
$filename=uploadurladmin.'export/hidrantesPorMunicipio.xls';
