	...
	*/
	...
	if (isset($_POST['enviar'])) 
	{
		...
		$campos = array (
			...
			"telefonos" => strip_tags($datos['telefonos']),
		...
		$action_edit=$bd->get_all_by_id("usuarios","Name",$campos['Name']);
		...
		if (!$action_edit)
		{
			if (!empty($datos['Name']))
			{
				if (!empty($datos['Password']))
				{
					...
					echo '<div id="error"><p>'.$str_lang['LANG_MSG_ERROR2'].'</p></div>';
			}else
			echo '<div id="error"><p>'.$str_lang['LANG_MSG_ERROR3'].'</p></div>';
		}else
		{
			echo '<div id="error"><p>'.$str_lang['LANG_MSG_ERROR4'].'</p></div>';
		}
...
?>
	...
	<div id="content">
		...
		<div class="header-doc">
			<h1><span><?=$str_lang['LANG_INSERT_TITLE_LISTADO']?></span></h1>	
            <div class="clr"><hr /></div>
			<p class="head-button"><a href="<?php echo $admin_path?>usuarios/"><span><?=$str_lang['LANG_INSERT_VOLVER']?></span></a></p>			
			<div class="clr"><hr /></div>
		</div>
					...
					<table id="table-parte">
						<tbody>					
							<tr>
								...
								<td ><input type="text" name="nombre" id="nombre" class="large" value="" maxlength="150"/></td>
							</tr>
							<tr>
								<td class="title" ><?=$str_lang['LANG_USERS_COL_LASTNAME']?>:</td>
								<td ><input type="text" name="apellidos" id="apellidos" class="large" value="" maxlength="150"/></td>
							</tr>	
							<tr>
								<td class="title" ><?=$str_lang['LANG_USERS_COL_COD']?>:</td>
								<td ><input type="text" name="codigo" id="codigo" class="large" value="" maxlength="50"/></td>
							</tr>
							<tr>
							<?php
							...
							<tr>
								...
								<td ><input type="text" name="telefonos" id="telefonos" class="large" value="" maxlength="200"/></td>
							</tr>	
							<tr>
								<td class="title" ><?=$str_lang['LANG_USERS_COL_EMAIL']?>:</td>
								<td ><input type="text" id="email" name="email" class="large" value="" maxlength="100"/></td>
							</tr>
							<tr>
								<td class="title" ><?=$str_lang['LANG_USERS_COL_USER']?>:</td>
								<td ><input type="text" id="Name" name="Name" class="short" value="" maxlength="100"/></td>							
							</tr>	
							<tr>
							<?php
								...
								<?php
									...
									echo cargarCombo("-1",$vector,"perfil_id","perfil".$sub);
								?>
								</select>
								</td>
							</tr>	
						...
						<p class="button" style="margin-top:20px;text-align:center">
							...
							<input type="submit" value="<?=$str_lang['LANG_INSERT_GUARDAR']?>" class="button" id="enviar" name="enviar"/>
						</p>                					
				</div>
				<div class="clr"><hr /></div>
			</div>
